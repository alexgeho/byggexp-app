> ⚠️ АРХИВ. Актуальный рабочий файл — **`WORKLOG.md`** (в корне репо). Здесь только история прошлых сессий.
...
Итеративная доводка по фидбэку Alexander + дизайнера Wlad (переписка в Telegram):

- **Мягкое голубое свечение** за мокапом = Figma «Ellipse 20» (`#4CABFF`, взял точный спек из файла `5DUxY…`: W393×H190, 14%). Сделано `react-native-svg` RadialGradient (тот же приём, что `cardGlow` в `MainButtonsGrid`). Итог: `SlideGlow` внутри `mockWrap`, **шире** мокапа (спилл по бокам, `left/right:-56`), но **короче** по высоте (`top/bottom:28%`) — чтобы НЕ вылезал за верх/низ и не давал светлую полосу-зазор. Пик 0.16.
- **Затухание всех 4 краёв мокапа** (`MockFade`): BG-градиенты по краям → мокап бесшовно втекает в фон, нет жёстких краёв/«огрызков» в углах. Низ усилен: `FADE_BOTTOM` height **80**, `bottom:-2` (чтобы не было полоски у нижней кромки). Верх/бока — 44/40.
- **Кнопки — эволюция стиля** (по фидбэку): тяжёлая синяя → светлая → стеклянный градиент как Home-карточки → **ghost с бледным бордером** `rgba(7,133,244,0.30)` + синий текст + иконка Feather `arrow-right`. **Последний слайд** («Kom igång» / get started) = **акцентная сплошная синяя** (`ghostCta = admin && !isLast`).
- **Мокап −10%** (`mockWrapAdmin: transform scale 0.9`) — просьба дизайнера.
- **Центрирование заголовка** между иллюстрацией и футером: заголовок ВНУТРИ `heroWrap` (фикс-высота `flex:1`) между двумя `flexSpacer` (flex:1). Спейсеры схлопываются в 0 → **наложение на высокие мокапы невозможно**. Это финальное решение после нескольких неудачных (space-evenly / titleWrap flexGrow — ломали измерение высоты высоких мокапов → текст налезал).
- **Метод проверки:** HTML-реплики реальной flexbox-раскладки в Chrome (react-native-web = тот же Yoga). Ловит layout-баги до OTA. Файлы реплик в scratchpad (`slide.html`, `cmp.html`, `cmp2.html`).

Последний OTA этой серии: group `59d73491-47b9-4872-b87a-d0b8d13c2052`, commit `8196959`.

## 🔜 NÄSTA STEG (продолжить тут)

1. **Проверить нижнее затухание 80px на устройстве** после перезапуска — особенно слайд **Ekonomi/costs**: строка «Marginal (18%) 140 944,48 kr» стоит у самого низа карточки, сильное затухание её подтеняет. Если «соль» слайда теряется → уменьшить `FADE_BOTTOM.height` до ~56 (компромисс), ИЛИ добавить нижний паддинг в `CostsMock` карточке, чтобы затухание ело пустоту, а не число.
2. **Прогнать ВСЕ 6 админ-слайдов** (arbetspass/employees/notification/receipt/costs/documents) на устройстве: центрирование текста + отсутствие полоски снизу + свечение по центру.
3. При желании — распространить ghost/акцентную кнопку и стиль на **воркер-слайды** (сейчас только админ) + на «Kom igång» чеклист для единообразия.
4. Отдельная ветка работы (не трогали сегодня): **iOS-билд 182 ждёт сабмита в TestFlight** — см. запись ниже.

## ⚠️ Öppna frågor

- Баланс нижнего затухания на Ekonomi-слайде (полоска vs читаемость «Marginal») — ждёт вердикта Alexander после перезапуска.
- Figma-файл с онбордингом у Alexander = `5DUxYAlucudqDuxoRDoWrC` («Алекс Герхард»), но мой REST-токен видит только дубль `UudMBHSSsCsBpq6m1z3B0X` («Алекс Герхард (18 Y)»). Спеки совпадают; если понадобится точный доступ к `5DUxY` — расшарить на токен.
...
1. **Онбординг-карточка («2 av 4 klara») перенесена вниз** (commit `a172d586`, OTA group `0dff6a7f`):
   - Была в самом верху ScrollView (конкурировала с селектором проекта за первый фокус). Теперь **между таймером и кнопками действий (Play + камера)**. Новый порядок Home: селектор проекта → таймер → **онбординг** → круглые/квадратные кнопки → быстрые кнопки → секции.
   - Технически: блок `<HomeOnboarding>` вырезан из верха ScrollView и вставлен внутрь `coreControlsGroup` после `timerSlot` (перед `MainActionButtons`). `coreControlsGroup` = `alignItems:"stretch"` → карточка на полную ширину. Заказ Александра — выбрал вариант «под таймер, кнопки ниже».
2. **Нав-пилюля на Home возвращена к оригинальному frosted glass** (commit `3296092a`, OTA group `51f91b5e`) — Александр: «верни пилюлю на айфоне как была». **Откат редизайна `pillColor`** от сессии 2026-09-10:
   - `HomeVariant2` `<BottomBar>`: убран `pillColor`, возвращены пропсы `glass` + `darkOverride={themeName==="black"}` + оригинальный `iconColor` (black→#FFFFFF / lightBlue→text / иначе `#052D50`).
   - `BottomBar.styles.js` `menuWrapperGlass`: яркое кольцо назад — `borderWidth:2`, `#FFFFFF` (Android) / `rgba(255,255,255,0.30)` (iOS). Было приглушённое `0.16`.
   - `BottomBar.jsx`: `menuShadow` больше НЕ применяется к glass-варианту (`!isTransparent && !glass`) — у Home тени раньше не было.
   - **Внутренние экраны НЕ тронуты** — там остаются тень + neutral hairline рамка (тот «гораздо лучше» вид на белом из прошлой сессии). `glass` использует только `HomeVariant2`, поэтому откат кольца/тени = Home-only.

### ⏭️ Следующие шаги

1. **Свериться на устройстве** после 2 перезапусков (последний OTA — group `51f91b5e`, iOS+Android): (а) онбординг стоит под таймером, читается, кнопки Play+камера под ним не перекрыты; (б) нав-пилюля на Home снова frosted glass с ярким кольцом (не сплошная цветная), внутренние экраны — с тенью как были.
2. Если после переноса онбординга низ Home стал тесным (кнопки/секции жмутся) — проверить распределение на коротких экранах (`shouldDistributeBlocksEvenly`).
3. Дальше — незакрытый бэклог ниже (Play vc22 device-QA + shared-компоненты + обед→зарплата + magic-login language + iOS-стиль остальных экранов + dark-theme палитра).
