...
- [ ] Создание задачи: свериться с админкой (функционал + правила) и привести мобилу к тому же — в т.ч. блок Heldag (зачёркнут на скрине). РАЗОБРАНО, НЕ СДЕЛАНО: в админке нет Heldag; нет в мобиле `priority` (low/normal/high) и `recurrence` (none/daily/weekdays/weekly/biweekly/monthly) — оба есть в бэкенде; админка предзаполняет даты сегодня + дефолт 08:00 старт / 17:00 дедлайн
