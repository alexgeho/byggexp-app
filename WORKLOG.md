...
- [ ] Создание задачи: свериться с админкой (функционал + правила) и привести мобилу к тому же — в т.ч. блок Heldag (зачёркнут на скрине)
- [ ] Lägg till verktyg: добавить сканирование QR (логика уже есть в ToolScanScreen — перенести сюда: добавить инструмент, отсканировав его код)
- [x] БАГ: создал инструмент → «Verktyg skapat», а списка нет (ProjectAdmin видел только инструменты своих проектов; у нового инструмента projectIds пуст) — бэкенд `530701f`
- [ ] БАГ: «Koppla till projekt» / «Koppla till arbetare» пусты — `/users/role/worker` только для superadmin → 403 → Promise.all обнулял И проекты
- [ ] БАГ онбординга: отмечены галочками пункты, которые Alexander НЕ делал — företagsuppgifter, klient, offert/faktura, Anpassa startsidan (свежий аккаунт)
