...
export default function ExpenseReviewSheet({ visible, asset, scanned, shift, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.message || 'Kunde inte spara utlägg');
    } finally {
      setSaving(false);
    }
  };
      ...
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <TextInput style={styles.input} value={supplier} onChangeText={setSupplier} placeholder="t.ex. Byggmax" />

            <View style={styles.row}>
              <View style={styles.col}>
                <Text style={styles.label}>Totalt (SEK)</Text>
                <TextInput style={styles.input} value={total} onChangeText={setTotal} keyboardType="decimal-pad" placeholder="0" />
              </View>
              <View style={styles.col}>
                <Text style={styles.label}>Moms (SEK)</Text>
                <TextInput style={styles.input} value={vat} onChangeText={setVat} keyboardType="decimal-pad" placeholder="0" />
              </View>
            </View>

            <Text style={styles.label}>Kategori</Text>
            <TextInput style={styles.input} value={category} onChangeText={setCategory} placeholder="t.ex. Material" />
            ...
            {hasShift && shift?.projectName ? (
              ...
              <View style={styles.projectList}>
                {projects.length === 0 ? (
                  <Text style={styles.hint}>Inga projekt hittades</Text>
                ) : (
                  projects.map((p) => {
                    const id = p._id || p.id;
          ...
          <View style={styles.actions}>
            ...
            <TouchableOpacity style={styles.saveBtn} onPress={save} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveText}>Spara utlägg</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </View>
