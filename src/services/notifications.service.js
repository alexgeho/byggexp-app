...
const buildNavigationTarget = (data = {}) => {
  ...
  if (screen === "Task") {
    const taskId = data.taskId || data.entityId || data.id;
...
const requestPushToken = async () => {
  ...
  if (!projectId) {
    throw new Error(
      "Expo projectId is missing. Push notifications require extra.eas.projectId.",
    );
  }
...
export const unregisterPushToken = async () => {
  ...
  try {
    await api.delete(
      `/notifications/push-token/${encodeURIComponent(installationId)}`,
    );
    await safeLogActivity({
      category: "notifications",
